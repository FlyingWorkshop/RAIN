...
class Forest:
    ...
    def _action_to_xy(self, action):
        """Helper function used in higher classes"""
        action = (action + 1) / 2
        x, y = int(self.world.shape[1] * action[1]), int(self.world.shape[0] * action[0])
        return x, y
    
    def _xy_to_action(self, x, y):
        """Helper function used in higher classes"""
        a0 = (y / self.world.shape[0]) * 2 - 1
        a1 = (x / self.world.shape[1]) * 2 - 1
        action = np.array([a0, a1])
        return action
    
    def _compute_action_rect(self, action):
        x, y = self._action_to_xy(action)
    ...
    def step(self, action=None):
        ...
        if action is not None:
            ...
            if action.ndim == 2:  # TODO: remove this hack somehow
                action = action[0]
            action = (action + 1) / 2
            x, y = int(self.world.shape[1] * action[1]), int(self.world.shape[0] * action[0])
            ...
            aimed_fire = np.sum(self.fire[y_1:y_2, x_1:x_2])
            self.world[y_1:y_2, x_1:x_2] = np.where(self.world[y_1:y_2, x_1:x_2] == self.FIRE_CELL, self.EMPTY_CELL,
                                                    self.world[y_1:y_2, x_1:x_2])  # extinguish fire in that cell
        else:
            self.action_rect = None
