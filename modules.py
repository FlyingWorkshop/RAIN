...
class XEncoder(nn.Module):
    def __init__(self, config):
        ...
        elif config.x_encoder == "cnn":
            self.f = CNN(
                output_dim=config.x_transf_dim
            )
...
class LatentEncoder(nn.Module):
    ...
    def forward(self, R, knowledge, n):
        ...
        if self.config.knowledge_merge == "sum":
            mean_k = torch.mean(k, dim=0, keepdim=True)
            encoder_input = F.relu(R + mean_k)
...
class Decoder(nn.Module):
    ...
    def forward(self, x_target, R_target):
        ...
        R_target [num_samples, bs, num_targets, hidden_dim] (1 1 1 128)
        x_target [bs, num_targets, input_dim] (1 1 128)
        """
        R_target = R_target.mean(dim=2, keepdims=True)  # TODO: revisit
